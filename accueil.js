function signin() {
    ...
    $.post('http://127.0.0.1:5000/signin', {
        ...
        if (data.result) {
            // Connexion réussie, rediriger vers la page profil
            window.location.href = '/';
        } else {
            // Afficher message d'erreur (facultatif)
            displayBox("ERROR", data.error);
        }
    }, 'json').fail(function() {
        // Gérer les erreurs de requête AJAX (facultatif)
        displayBox("ERROR", "Erreur lors de la connexion au serveur.");
    });
...
function register() {
    ...
    return false;  // Empêche le rechargement de la page (par le bouton)
}
...
function signout() {
    ...
    .fail(function(jqXHR, textStatus, errorThrown) {
        displayBox("ERROR", "Signout failed: " + textStatus);
    });
